...
if __name__ == '__main__':
    ...
    max_time = 5
    hifi_dt = .1
    hifi_max_path_length = int(np.ceil(max_time / hifi_dt))
    lofi_dt = .5
    lofi_max_path_length = int(np.ceil(max_time / lofi_dt))
...
if RUN_LOFI:
    ...
    # simulation settings
    from examples.hifi.simulators.time_av_simulator import TimeAVSimulator
    sub_sim_args = {'dt':lofi_dt,}
    sim_args = {'blackbox_sim_state': blackbox_sim_state,
                'open_loop': open_loop,
                'fixed_initial_state': fixed_init_state,
                'max_path_length': lofi_max_path_length,
                'simulator_cls':TimeAVSimulator,
    ...
    drl_algo_args = {'max_path_length': lofi_max_path_length,
                     'discount': 0.99,
                     'lr_clip_range': 1.0,
                     'max_kl_step': 1.0,
                     # 'log_dir':None,
...
if RUN_BA:
    # simulation settings
    from examples.hifi.simulators.time_av_simulator import TimeAVSimulator
    sub_sim_args = {'dt':hifi_dt,}
    sim_args = {'blackbox_sim_state': blackbox_sim_state,
                'open_loop': open_loop,
                'fixed_initial_state': fixed_init_state,
                'max_path_length': hifi_max_path_length,
                'simulator_cls':TimeAVSimulator,
                'simulator_args':sub_sim_args,
                }
    # BA Settings
...
if RUN_HIFI:
    ...
    drl_algo_args = {'max_path_length': hifi_max_path_length,
                     'discount': 0.99,
                     'lr_clip_range': 1.0,
                     'max_kl_step': 1.0,
                     # 'log_dir':None,
