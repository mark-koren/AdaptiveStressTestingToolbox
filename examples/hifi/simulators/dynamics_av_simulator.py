...
class DynamicsAVSimulator():
    ...
    def step_simulation(self, action):
        ...
        noise = np.around(self._action.reshape((self.c_num_peds, 6))[:, 2:6], decimals=self.decimals)
    ...
    def sensors(self, peds, noise):
        ...
        return np.around(measurements, decimals=self.decimals)
    ...
    def tracker(self, estimate_old, measurements):
        ...
        return np.around(observation, decimals=self.decimals)
    ...
    def update_car(self, obs, v_car):
        ...
        if np.isnan(a):
            pdb.set_trace()
        # pdb.set_trace()
        return np.around(np.clip(a, -self.c_d_max, self.c_a_max), decimals=self.decimals)
    ...
    def move_car(self, car, accel):
        ...
        return np.around(car, decimals=self.decimals)
    ...
    def update_peds(self):
        ...
        action = np.around(self._action.reshape((self.c_num_peds, 6))[:, 0:2], decimals=self.decimals)
        ...
        self._peds = np.around(self._peds, decimals=self.decimals)

    def observe(self):
        """Get the ground truth state of the pedestrian relative to the ego vehicle.

        """
        self._env_obs = np.around(self._peds - self._car, decimals=self.decimals)
