...
class PerceptionAVReward(ASTReward):
    """An example implementation of an ASTReward for an AV validation scenario.
    ...
    def mahalanobis_d(self, action):
        ...
        """
        # Mean action is 0
        mean = np.zeros((3 * self.c_num_peds, 1))
        # Assemble the diagonal covariance matrix
        cov = np.zeros((self.c_num_peds, 3))
        cov[:, 0:3] = np.array([self.c_cov_x, self.c_cov_y,
                                self.c_cov_sensor_noise,])
